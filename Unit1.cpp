...
void TForm1::ShapeCreate(int type)
{
	...
	Lines[i] = new TLine(this);
	Lines[i]->Parent = ScrollBox1;
	Lines[i]->Position->X = block[i==0?0:i-1]->Position->X + block[i==0?0:i-1]->Size->Width/2;
	Lines[i]->Position->Y = block[i==0?0:i-1]->Position->Y + block[i==0?0:i-1]->Size->Height;
	Lines[i]->Size->Width = 1;

	/*
	line->Position->X = block[i]->Position->X + block[i]->Size->Width/2;
...
void __fastcall TForm1::Example7Click(TObject *Sender)
{
	...
	{
		ShapeCreate(7);
		block[m]->Position->X=block[m-1]->Position->X+block[m-1]->Width+40;
		block[m]->Position->Y=block[m-1]->Position->Y;
		Image2->Visible=false;
		Apply->Visible=false;
		Delete->Visible=false;
		Selectif->Visible=false;
